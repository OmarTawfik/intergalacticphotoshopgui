...
extern "C" DllExport void AddRayleighNoiseExecute(ImageData source, int* red, int* green, int* blue, double percentage, double sigma)
{
...
#pragma omp parallel for shared(red, green, blue, percentage, src, remaining, sigma, redMax, greenMax, blueMax) private(p, i, x, y, count, dist)
	for ( i = 0 ; i < 256; i ++)
	{
		dist =  i * exp(-i * i / (2 * sigma * sigma)) / (sigma * sigma);
