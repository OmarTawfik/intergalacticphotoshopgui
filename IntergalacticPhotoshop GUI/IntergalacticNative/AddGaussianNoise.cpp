...
extern "C" DllExport void AddGaussianNoiseExecute(ImageData source, int* red, int* green, int* blue, double percentage, double mean, double sigma)
{
	...
	double dist, root = sqrt(2.0 * PI * sigma * sigma);

#pragma omp parallel for shared(root, red, green, blue, percentage, src, remaining, mean, sigma, redMax, greenMax, blueMax) private(p, i, x, y, count, dist)
	for ( i = 0 ; i < 256; i ++)
	{
		dist =  exp(-(i - mean) * (i - mean) / (2 * sigma * sigma)) / root;
