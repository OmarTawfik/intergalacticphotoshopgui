...
extern "C" DllExport void HorizontalEdgeDetectionOperationExecute(ImageData src, ImageData dest)
{
    ...
    for (i = 0; i < src.Height; i++)
    {
        for (j = 0; j < src.Width; j++)
        {
			...
			red += (leftUp->R * 5) + (up->R * 5) + (rightUp->R * 5);
            green += (leftUp->G * 5) + (up->G * 5) + (rightUp->G * 5);
            blue += (leftUp->B * 5) + (up->B * 5) + (rightUp->B * 5);

            red += -(left->R * 3) - (right->R * 3) - (leftDown->R * 3) - (down->R * 3) - (rightDown->R * 3);
            green += -(left->G * 3) - (right->G * 3) - (leftDown->G * 3) - (down->G * 3) - (rightDown->G * 3);
            blue += -(left->B * 3) - (right->B * 3) - (leftDown->B * 3) - (down->B * 3) - (rightDown->B * 3);
