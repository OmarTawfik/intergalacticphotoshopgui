...
extern "C" DllExport void MeanFilter2DOperationExecute(ImageData src, ImageData dest, int maskSize)
{
	...
	#pragma omp parallel for shared(src, dest, side, maskSize) private(i, j, newX, newY, a, b, red, green, blue, p) 
    for (i = 0; i < src.Height; i++)
    {
        for (j = 0; j < src.Width; j++)
        {
			...
			p->R =  (unsigned char)(red / (maskSize * maskSize));
			p->G = (unsigned char)(green / (maskSize * maskSize));
            p->B = (unsigned char)(blue / (maskSize * maskSize));
        }
    }
}            
