...
extern "C" DllExport void AddUniformNoiseExecute(ImageData source, int* red, int* green, int* blue, double percentage, int start, int end)
{
...
#pragma omp parallel for shared(red, green, blue, percentage, src, remaining, start, end, redMax, greenMax, blueMax) private(p, i, x, y, count, dist)
	for ( i = 0 ; i < 256; i ++)
	{
		dist = (start <= i && i < end) ? 1.0 / (end - start) : 0;
